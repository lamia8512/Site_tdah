    ...
    <form method="POST">
        ...
        <div class="container">
            ...
            <div class="form-group">
                ...
                <!-- Cette ligne crée un champ de formulaire qui affiche le titre actuel de l’article et permet à l’utilisateur de le modifier en envoyant la nouvelle valeur -->
                ...
                if(isset($this->errors['article'])){
                    ?>
                        <!-- Dans un éccho php, on affiche l'erreur qu'il y a dans le contenu de l'article en rouge -->
                        <p class="text-danger"><?= $this->errors['article'] ?></p>
                    <?php
                }
                ?>
            </div>
            <!-- Création d'un bouton jaune de type sumbit pour envoyer les nouvelles données du formulaire au serveur (name permet de récupérer la méthode editArticle dans ArticleController) -->
            <button type="submit" name="editArticle" class="btn btn-warning my-4">Modifier !</button>
        </div>
    </form>
</main>
